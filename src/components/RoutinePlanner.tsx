...
export const RoutinePlanner: React.FC = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-8">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-md flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="space-y-1">
          <h1 className="text-2xl font-bold text-slate-100 flex items-center gap-2">
            ...
            Rotina Visual & Lembretes Inteligentes
          </h1>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          ...
          >
            {notificationsAllowed ? <BellRing className="w-4 h-4 text-teal-400" /> : <Bell className="w-4 h-4" />}
            <span>{notificationsAllowed ? "Lembretes Ativos" : "Ativar Notificações"}</span>
          </button>

          {/* Sensory Timer Widget */}
          <div className="bg-slate-950 border border-slate-800 rounded-2xl p-3 flex items-center gap-3 text-xs sm:text-sm shadow-inner">
            <div className="text-center">
              <span className="text-[10px] uppercase font-bold text-teal-400 tracking-wider">
                {timerMode === "foco" ? "Foco" : "Pausa"}
              </span>
              <div className="text-xl font-extrabold font-mono text-slate-100">{formatTime(timerSeconds)}</div>
            </div>
      ...
      <div className="bg-slate-900/90 border border-teal-800/60 rounded-2xl p-5 space-y-4 shadow-md">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 px-4 py-2.5 bg-slate-950 border border-slate-700 rounded-xl text-slate-100 text-xs sm:text-sm focus:outline-none focus:border-teal-500"
          />
          <button
        ...
        {generatedSteps.length > 0 && (
          <div className="p-4 bg-slate-950 border border-slate-800 rounded-xl space-y-2">
            <h4 className="text-xs font-bold text-teal-400 uppercase tracking-wider">Passos Sugeridos para Adicionar à Rotina:</h4>
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-4">
          {filteredTasks.length === 0 ? (
            <div className="p-8 text-center bg-slate-900/60 border border-slate-800 rounded-2xl text-slate-400 text-xs sm:text-sm">
              ...
              >
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                  ...
                  <div className="flex items-start gap-4">
                    <button
                      ...
                      className={`p-2 rounded-xl transition flex-shrink-0 mt-1 ${
                        task.completed ? "bg-emerald-950 text-emerald-400" : "bg-slate-800 text-slate-400 hover:text-slate-200"
                      }`}
                    >
                    ...
                    {task.imageUrl && (
                      <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-xl overflow-hidden border border-slate-700 flex-shrink-0 bg-slate-950">
                    ...
                    <div className="space-y-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {task.reminderTime && (
                          <span className="text-[11px] font-mono font-bold text-amber-300 bg-amber-950/80 border border-amber-800 px-2 py-0.5 rounded-full flex items-center gap-1">
                            <Clock className="w-3 h-3" /> {task.reminderTime}
                          </span>
                        )}
                      </div>
                      <span className="text-[11px] text-teal-400 font-medium capitalize block">
                  ...
                  <button
                    ...
                    className="p-2 text-slate-500 hover:text-rose-400 transition self-end sm:self-center"
                  >
                ...
                {task.steps && task.steps.length > 0 && (
                  <div className="pl-11 space-y-1">
                    <p className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider">Etapas:</p>
        ...
        <div className="p-5 bg-slate-900 border border-slate-800 rounded-2xl space-y-4 shadow-lg">
          <h4 className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
            ...
            Adicionar Nova Atividade com Foto na {activeSlot}:
          </h4>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <div className="flex gap-2">
              ...
              <input
                ...
                className="px-2 py-2 bg-slate-950 border border-slate-700 rounded-xl text-xs font-mono text-slate-100"
          ...
          <div className="space-y-2 pt-2 border-t border-slate-800">
            <label className="block text-xs font-semibold text-slate-300">Escolha uma Foto de Referência Visual ou Faça Upload:</label>
                ...
                <button
                  ...
                  className={`flex-shrink-0 flex items-center gap-2 p-1.5 rounded-xl border text-xs transition ${
                    newImageUrl === preset.url
                      ? "bg-teal-950 border-teal-500 text-teal-200 ring-2 ring-teal-500"
            ...
            <div className="flex items-center gap-3">
              <label className="cursor-pointer px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 rounded-xl text-xs font-semibold flex items-center gap-2 border border-slate-700 transition">
              ...
              {newImageUrl && (
                <div className="flex items-center gap-2 text-xs text-teal-400">
                  ...
                  <button onClick={() => setNewImageUrl("")} className="text-slate-500 hover:text-rose-400">
                    Remover
                  </button>
                </div>
              )}
